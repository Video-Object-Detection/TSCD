...
def extract_values(text):
    ...
    try:
        AP_small = re.search(r'Average Precision  \(AP\) @\[ IoU=0.50:0.95 \| area= small.*? \] = (\d+\.\d+)',
                             text).group(1)
        AP_medium = re.search(r'Average Precision  \(AP\) @\[ IoU=0.50:0.95 \| area=medium.*? \] = (\d+\.\d+)',
                              text).group(1)
        AP_large = re.search(r'Average Precision  \(AP\) @\[ IoU=0.50:0.95 \| area= large.*? \] = (\d+\.\d+)',
                             text).group(1)
        AR1 = re.search(r'Average Recall     \(AR\) @\[ IoU=0.50:0.95 \| area=   all \| maxDets=  1 \] = (\d+\.\d+)',
                        text).group(1)
        AR10 = re.search(r'Average Recall     \(AR\) @\[ IoU=0.50:0.95 \| area=   all \| maxDets= 10 \] = (\d+\.\d+)',
                         text).group(1)
        AR100 = re.search(r'Average Recall     \(AR\) @\[ IoU=0.50:0.95 \| area=   all \| maxDets=100 \] = (\d+\.\d+)',
                          text).group(1)
        AR_small = re.search(r'Average Recall     \(AR\) @\[ IoU=0.50:0.95 \| area= small.*? \] = (\d+\.\d+)',
                             text).group(1)
        AR_medium = re.search(r'Average Recall     \(AR\) @\[ IoU=0.50:0.95 \| area=medium.*? \] = (\d+\.\d+)',
                              text).group(1)
        AR_large = re.search(r'Average Recall     \(AR\) @\[ IoU=0.50:0.95 \| area= large.*? \] = (\d+\.\d+)',
                             text).group(1)
    except Exception:
        AP_small = 0
        AP_medium = 0
...
class Trainer:
    ...
    def train_one_iter(self):
        ...
        inps, targets, time_embedding = self.prefetcher.next()
        if inps is None:
            logger.info(str(np.array(targets)), str(np.array(time_embedding)))
        inps = inps.to(self.data_type)
        time_embedding = time_embedding.to(self.data_type)
        ...
        with torch.cuda.amp.autocast(enabled=self.amp_training):
            outputs = self.model(inps, targets, time_embedding, lframe=self.exp.lframe, gframe=self.exp.gframe,
                                 resume=False)
    ...
    def after_iter(self):
        ...
        if (self.iter + 1) % self.exp.print_interval == 0:
            ...
            for k, v in loss_meter.items():
                if k in self.loss_info:
                    self.loss_info[k] += v.latest
                else:
                    self.loss_info[k] = v.latest
        ...
        if self.iter % 1000 == 0:
            loss_str = ", ".join(
                ["{}: {:.1f}".format(k, v / 100) for k, v in self.loss_info.items()]
            )
            logger.info("{}".format(loss_str))
            self.loss_info = {}
