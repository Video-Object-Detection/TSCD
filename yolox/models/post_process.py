...
def postprocess(prediction, num_classes, fc_outputs,
                conf_output, reg_output, conf_thre=0.001, nms_thre=0.5,
    ...
    for i, detections in enumerate(prediction):
        ...
        # [:,cls_loc[1]]
        # tmp_cls_score[torch.stack(cls_loc).T]#torch.gather(tmp_cls_score, dim=1, index=torch.stack(cls_loc).T)
...
def post_linking(fc_outputs, adj_lists, pred_results, P, Cls, names, exp):
    ...
    for i in range(splits):
        ...
        result, _ = postprocess(copy.deepcopy(pred_result), Cls, fc_output_split, None, nms_thre=exp.nmsthre,
                                cls_sig=False)
