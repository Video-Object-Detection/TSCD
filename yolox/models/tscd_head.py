...
from yolox.models.post_trans import LocalAggregation, MCA_tscd_g2l_reg, MCA_tscd_g2l_cls
from .surrounding_extraction import WaveletsHFBlock
from yolox.utils import bboxes_iou
from yolox.utils.box_op import box_cxcywh_to_xyxy, generalized_box_iou
from .losses import IOUloss
from .network_blocks import BaseConv, DWConv
from .tscd_matching import AwarePositionRegMatcher, TaskAligned
from yolox.utils.debug_vis import visual_predictions
from matplotlib import pyplot as plt


class TSCDHead(nn.Module):
    """
        TSCD Head Module
    """
    ...
    def __init__(
        ...
        if kwargs.get('ota_mode', False):
            if kwargs.get('agg_type', 'localagg') == 'localagg':
            ...
            elif kwargs.get('agg_type', 'localagg') == 'mca':
                """
                ...
                """
                self.agg = MCA_tscd_g2l_reg(dim=self.width, out_dim=4 * self.width,
                ...
                if kwargs.get('decouple_reg', False):
                    """
                        ...
                        Frame-by-frame enhances the local feature.
                    """
                    self.agg_iou = MCA_tscd_g2l_reg(dim=self.width, out_dim=4 * self.width,
                    ...
                    """
                    self.local_reg_matcher = AwarePositionRegMatcher(hidden_channel=self.width,
                                                                     feedforward_channel=self.width,
                                                                     num_head=8, decoder_layer_num=1, act=act)
                    self.fc_reg_matcher = nn.Linear(self.width, 4 * self.width)
                    """
                        multi-head cross-attention
                        to align the classification and regression tasks
                    """
                    self.task_aligned = TaskAligned(hidden_channel=4 * self.width, num_head=8,
                                                    decoder_layer_num=1, act=act)
                self.cls_pred = nn.Linear(4 * self.width, num_classes)
                if kwargs.get('reconf', False):
                    self.matcher_obj_pred = nn.Linear(4 * self.width, 1)
                    self.matcher_reg_pred = nn.Linear(4 * self.width, 4)
                self.cls_convs2 = nn.ModuleList()
        else:
            if kwargs.get('reconf', False):
                self.agg = LocalAggregation(dim=self.width, heads=heads, attn_drop=drop, blocks=localBlocks,
        ...
        for i in range(len(in_channels)):
            ...
            """
            self.edge_enhance_reg.append(
                nn.Sequential(
                    *[
                        WaveletsHFBlock(in_channels=int(256 * width)),
                    ]
                )
            )
    ...
    def forward(self, xin, labels=None, imgs=None, time_embedding=None, nms_thresh=0.5, lframe=0, gframe=32,
                resume=False):
        outputs = []
        outputs_decode = []
        origin_preds = []
        ...
        for k, (cls_conv, reg_conv, stride_this_level, x) in enumerate(
            ...
            if self.kwargs.get('vid_reg', True):
            ...
            if self.kwargs.get('vid_reg', True):
                ...
                # extract the surrounding high frequency features
                raw_reg_edge_features.append(self.edge_enhance_reg[k](vid_feat_reg))
            else:
                raw_reg_features.append(reg_feat)
                # extract the surrounding high frequency features
                raw_reg_edge_features.append(self.edge_enhance_reg[k](reg_feat))
        ...
        decode_res, _, _ = self.decode_outputs(outputs_decode, dtype=xin[0].type())

        preds_per_frame = []

        if self.training:
            # get foreground prediction idx
        ...
        # cls_edge_feat_flatten = torch.cat(
        #     [x.flatten(start_dim=2) for x in raw_cls_edge_features], dim=2
        # ).permute(0, 2, 1)
        reg_edge_feat_flatten = torch.cat(
            [x.flatten(start_dim=2) for x in raw_reg_edge_features], dim=2
        ).permute(0, 2, 1)
        ...
        (features_cls, features_reg, features_edge_reg, cls_scores,
         fg_scores, locs, all_scores) = self.find_feature_score(cls_feat_flatten,
                                                                agg_idx,
                                                                reg_feat_flatten,
                                                                reg_edge_feat_flatten,
                                                                imgs,
                                                                pred_result)
        ...
        elif self.kwargs.get('agg_type', 'localagg') == 'mca':
            kwargs = self.kwargs
            kwargs.update(
                {'lframe': lframe, 'gframe': gframe, 'afternum': self.Afternum, 'preds_per_frame': preds_per_frame})
            # The features in the global frames are aligned and complemented
            # to the keyframes by multi-head cross-attention in classification branch.
            features_cls, _ = self.agg(features_cls_raw, features_reg_raw, cls_scores, fg_scores,
                                       sim_thresh=self.sim_thresh,
                                       ave=self.ave, use_mask=self.use_mask, **kwargs)
            features_cls_matched = features_cls
            # get the feature selected prediction nums per frame
            local_pred_num = 0
            for i in range(lframe):
                local_pred_num += preds_per_frame[i]
            if self.kwargs.get('decouple_reg', False):
                # The features in the global frames are aligned and complemented
                # to the keyframes by multi-head cross-attention in regression branch.
                features_cls, features_reg = self.agg_iou(features_cls_raw, features_reg_raw, cls_scores, fg_scores,
                                                          sim_thresh=self.sim_thresh,
                                                          ave=self.ave, use_mask=self.use_mask, **kwargs)
                features_obj_refined = features_reg

                # reduces accumulation of noisy localization during long-term feature alignment
                # further refines feature matching and increases attention on matched objects
                features_reg_matched = self.local_reg_matcher(features_reg_raw[0], features_reg, features_cls,
                                                              features_edge_reg, preds_per_frame[:lframe],
                                                              time_embedding[:lframe], resume=resume)
                if features_reg_matched is None:
                    features_reg_matched = features_reg_raw[0, :local_pred_num]
                else:
                    features_reg_matched = features_reg_matched[-1]

                # to align the classification and regression tasks
                features_reg_matched = self.fc_reg_matcher(features_reg_matched)
                features_obj_refined = self.task_aligned(features_reg_matched, features_obj_refined,
                                                         preds_per_frame[:lframe])
                if features_obj_refined is None:
                    features_obj_refined = features_reg

            # features_reg_matched = features_reg_raw[0, :local_pred_num]

            cls_preds = self.cls_pred(features_cls_matched)
            if self.kwargs.get('reconf', False):
                obj_preds = None
                reg_preds = None
                matcher_obj_preds = self.matcher_obj_pred(features_obj_refined)
                matcher_reg_preds = self.matcher_reg_pred(features_reg_matched)
            else:
                obj_preds, reg_preds, matcher_obj_preds, matcher_reg_preds = None, None, None, None
        ...
        if self.training:
            ...
            if not self.ota_mode:
                ...
                refine_cls_targets, refine_reg_targets, refine_cls_masks, refine_obj_targets = None, None, None, None
                        ...
                        if not self.kwargs.get('cat_ota_fg', True):
                            ...
                            for i in range(lframe):
                                ...
                                if ota_idxs[i] != None and tmp_reorder != None and len(tmp_reorder):
                                    ...
                                    tmp_reg_targets = reg_targets[i][torch.stack(tmp_reorder)]
                                    refine_cls_targets.append(tmp_cls_targets)
                                    refine_reg_targets.append(tmp_reg_targets)
                            if len(refine_cls_targets):
                                refine_cls_targets = torch.cat(refine_cls_targets, 0)
                                refine_reg_targets = torch.cat(refine_reg_targets, 0)
                            else:
                                refine_cls_targets = torch.cat(cls_targets, 0).new_zeros(0, self.num_classes)
                                refine_reg_targets = torch.cat(reg_targets, 0).new_zeros(0, 4)
                    else:
                        (refine_cls_targets,
                         refine_cls_masks,
                         _,
            ...
            # get the feature selected local prediction nums
            local_preds_num = 0
            for i in range(lframe):
                local_preds_num += preds_per_frame[i]
            refine_obj_masks = refine_obj_masks[:local_preds_num, :]
            # get the feature selected local foreground nums
            num_fg_local = 0
            for i in range(lframe):
                num_fg_local += len(ota_idxs[i])
            # get the still object detector prediction results
            original_pred_result = []
            for i in range(lframe):
                if pred_result[i] is None or pred_result[i] == []:
                    continue
                local_frame_pred_result = pred_result[i][:, :4]
                original_pred_result.append(local_frame_pred_result)
            original_pred_result = torch.cat(original_pred_result, dim=0).detach()
            # get the matcher refined loss
            return self.get_matcher_refined_losses(
                outputs,
                cls_targets,
                reg_targets,
                obj_targets,
            ...
            if cls_preds is not None:
                if cls_preds.dim() == 3:
                    ...
                    cls_preds = cls_preds
            if matcher_obj_preds is not None:
                if matcher_obj_preds.dim() == 3:
                    obj_preds = matcher_obj_preds[-1]
                else:
                    obj_preds = matcher_obj_preds
            # if decode_matcher_reg_preds is not None:
            #     if decode_matcher_reg_preds.dim() == 3:
            #         reg_preds = decode_matcher_reg_preds[-1]
            #     else:
            #         reg_preds = decode_matcher_reg_preds
            #     pred_bbox = torch.zeros_like(reg_preds)
            #     # x1
            #     pred_bbox[:, 0] = reg_preds[:, 0] - reg_preds[:, 2] * 0.5
            #     # y1
            #     pred_bbox[:, 1] = reg_preds[:, 1] - reg_preds[:, 3] * 0.5
            #     # TODO  (note: "- 1" is correct; don't be fooled by the asymmetry)
            #     # x2
            #     pred_bbox[:, 2] = reg_preds[:, 0] + reg_preds[:, 2] * 0.5
            #     # y2
            #     pred_bbox[:, 3] = reg_preds[:, 1] + reg_preds[:, 3] * 0.5
            #     reg_preds = pred_bbox
            if matcher_reg_preds is not None:
                if matcher_reg_preds.dim() == 3:
                    reg_preds = matcher_reg_preds[-1]
                else:
                    reg_preds = matcher_reg_preds
                original_pred_result = []
                for i in range(lframe):
                    if pred_result[i] is None or pred_result[i] == []:
                        continue
                    local_frame_pred_result = pred_result[i][:, :4]
                    original_pred_result.append(local_frame_pred_result)
                if len(original_pred_result) > 0:
                    original_pred_result = torch.cat(original_pred_result, dim=0)
                    # using the bias obtain from the video detector prediction, decode to obtain the prediction boxes
                    # dx dy dw dh -> x y x y
                    reg_preds = self.decode_reg_preds5(reg_preds, original_pred_result)
                else:
                    reg_preds = None
            # get the local frame predict results
            ...
            for i in range(lframe):
                if self.kwargs.get('reconf', False):
                    ...
                    if reg_preds is not None:
                        reg_per_frame.append(reg_preds[:preds_per_frame[i]])
                        reg_preds = reg_preds[preds_per_frame[i]:]
                    else:
                        reg_per_frame = None
            ...
            if not self.kwargs.get('reconf', False):
                obj_per_frame, reg_per_frame = None, None
            # obj_per_frame = None
            local_pred_result = [pred_result[i] for i in range(lframe)]
            # post process the predict results
            result, result_ori = postprocess(copy.deepcopy(local_pred_result),
                                             self.num_classes,
                                             cls_per_frame,
                                             conf_output=obj_per_frame,
                                             reg_output=reg_per_frame,
                                             nms_thre=nms_thresh,
                                             )
    ...
    def decode_outputs(self, outputs, dtype, flevel=0):
        ...
        return outputs, grids.squeeze(0), strides.squeeze(0)

    def decode_reg_preds1(self, reg_preds, agg_idx, dtype):
        """
            decode to get the true prediction boxes
            x y w h
        """
        grids = []
        strides = []
        scale = []
        for (hsize, wsize) in self.hw:
            scale.append(hsize * wsize)
        for batch_agg_idx in agg_idx:
            grid = []
            stride = []
            for i in range(len(batch_agg_idx)):
                index = batch_agg_idx[i]
                if index < scale[0]:
                    y = int(index / self.hw[0][0])
                    x = int(index % self.hw[0][1])
                    grid.append([x, y])
                    stride.append(self.strides[0])
                elif scale[0] <= index < (scale[0] + scale[1]):
                    index = index - scale[0]
                    y = int(index / self.hw[1][0])
                    x = int(index % self.hw[1][1])
                    grid.append([x, y])
                    stride.append(self.strides[1])
                else:
                    index = index - scale[0] - scale[1]
                    y = int(index / self.hw[2][0])
                    x = int(index % self.hw[2][1])
                    grid.append([x, y])
                    stride.append(self.strides[2])
            grids.append(torch.tensor(grid).type(dtype))
            strides.append(torch.tensor(stride).type(dtype))

        outputs = []
        for idx, reg_pred in enumerate(reg_preds):
            xy = (reg_pred[..., :2] + grids[idx]) * strides[idx].unsqueeze(1)
            wh = torch.exp(reg_pred[..., 2:4]) * strides[idx].unsqueeze(1)
            output = torch.cat((xy, wh), dim=-1)
            outputs.append(output)
        outputs = torch.stack(outputs, dim=0)

        return outputs

    def decode_reg_preds2(self, reg_preds, agg_idx, grids, strides, dtype):
        """
            decode to get the true prediction boxes
            x y w h
        """
        idxs = []
        for i, idx in enumerate(agg_idx):
            if idx is None or idx == []: continue
            idxs.append(idx)
        agg_idx = torch.cat(idxs, dim=0)
        outputs = []
        for _, reg_pred in enumerate(reg_preds):
            xy = (reg_pred[..., :2] + grids[agg_idx]) * strides[agg_idx]
            wh = torch.exp(reg_pred[..., 2:4]) * strides[agg_idx]
            output = torch.cat((xy, wh), dim=-1).type(dtype)
            outputs.append(output)
        outputs = torch.stack(outputs, dim=0)
        return outputs

    def decode_reg_preds3(self, reg_preds, preds_result, bbox_xform_clip=math.log(736. / 32)):
        """
            using the bias obtain from the video detector prediction, decode to obtain the prediction boxes
            dx dy dw dh -> x y x y
        """
        widths = preds_result[:, 2] - preds_result[:, 0]
        heights = preds_result[:, 3] - preds_result[:, 1]
        ctr_x = preds_result[:, 0] + 0.5 * widths
        ctr_y = preds_result[:, 1] + 0.5 * heights
        ...
        pred_bbox = torch.zeros_like(reg_preds)
        # x1
        pred_bbox[:, 0] = pred_ctr_x - 0.5 * pred_w
        # y1
        pred_bbox[:, 1] = pred_ctr_y - 0.5 * pred_h
        # x2
        pred_bbox[:, 2] = pred_ctr_x + 0.5 * pred_w
        # y2
        pred_bbox[:, 3] = pred_ctr_y + 0.5 * pred_h

        return pred_bbox

    def decode_reg_preds4(self, reg_preds, preds_result, bbox_xform_clip=math.log(736. / 32)):
        """
            using the bias obtain from the video detector prediction, decode to obtain the prediction boxes
            dx dy dw dh -> x y w h
        """
        ...
        ctr_x = ctr_x.unsqueeze(0)
        ctr_y = ctr_y.unsqueeze(0)
        ...
        pred_bbox = torch.stack([pred_ctr_x, pred_ctr_y, pred_w, pred_h], dim=2)
        # pred_bbox = torch.zeros_like(reg_preds)
        # # x1
        # pred_bbox[..., 0] = pred_ctr_x - 0.5 * pred_w
        # # y1
    ...
    def decode_reg_preds5(self, reg_preds, preds_result, bbox_xform_clip=math.log(736. / 32)):
        """
            using the bias obtain from the video detector prediction, decode to obtain the prediction boxes
            dx dy dw dh -> x y x y
        """
        ...
        # pred_bbox = torch.stack([pred_ctr_x, pred_ctr_y, pred_w, pred_h], dim=1)
        pred_bbox = torch.zeros_like(reg_preds)
        # x1
        pred_bbox[:, 0] = pred_ctr_x - 0.5 * pred_w
        # y1
    ...
    def encode_reg_preds(self, reg_targets, preds_result, eps=1e-8):
        """
           calculate the bias of the ground truth object boxes
           with respect to the predictions of the still object detector
           x y w h -> dx dy dw dh
        """
        gt_widths = reg_targets[:, 2]
        gt_heights = reg_targets[:, 3]
        gt_ctr_x = reg_targets[:, 0]
        gt_ctr_y = reg_targets[:, 1]
        ...
        targets_dw = torch.log(gt_widths / widths + eps)
        targets_dh = torch.log(gt_heights / heights + eps)

        targets = torch.stack((targets_dx, targets_dy, targets_dw, targets_dh), dim=1)

        return targets

    def find_feature_score(self, features, idxs, reg_features, reg_edge_features, imgs=None, predictions=None):
        """
            get scores for classification and regression foreground features and foreground features
        """
        features_cls = []
        features_reg = []
        features_edge_reg = []
        ...
        for i, feature in enumerate(features):
            ...
            features_edge_reg.append(reg_edge_features[i, idxs[i]])
        ...
        features_edge_reg = torch.cat(features_edge_reg)
        cls_scores = torch.cat(cls_scores)
        fg_scores = torch.cat(fg_scores)
        locs = torch.cat(locs)
        all_scores = torch.cat(all_scores)
        return features_cls, features_reg, features_edge_reg, cls_scores, fg_scores, locs, all_scores

    def get_matcher_refined_losses(
            self,
            outputs,
            cls_targets,
            reg_targets,
        ...
        """
            add bce loss for video classification, bce loss and smooth l1 loss for video localization based on yolox loss
        """
        ...
        if self.ota_mode:
            if self.kwargs.get('reconf', True):
                ...
                if matched_obj is not None:
                    if matched_obj.dim() == 3:
                        loss_matched_obj = 0
                        for i in range(matched_obj.size(0)):
                            loss_matched_obj += (
                                                    self.bcewithlog_loss(matched_obj[i].view(-1, 1),
                                                                         refined_obj_targets)
                                                ).sum() / num_fg_local
                        loss_matched_obj /= matched_obj.size(0)
                    else:
                        loss_matched_obj = (
                                               self.bcewithlog_loss(matched_obj.view(-1, 1), refined_obj_targets)
                                           ).sum() / num_fg_local
                else:
                    loss_matched_obj = 0
                # if matched_reg is not None:
                #     if matched_reg.dim() == 3:
                #         loss_matched_iou = 0
                #         for i in range(matched_reg.size(0)):
                #             loss_matched_iou += (
                #                                     self.iou_loss(matched_reg[i].view(-1, 4)[refined_obj_masks],
                #                                                   refined_reg_targets)
                #                                 ).sum() / num_fg_local
                #         loss_matched_iou /= matched_reg.size(0)
                #     else:
                #         loss_matched_iou = (
                #                                self.iou_loss(matched_reg.view(-1, 4)[refined_obj_masks],
                #                                              refined_reg_targets)
                #                            ).sum() / num_fg_local
                # else:
                #     loss_matched_iou = 0
                # smooth l1 loss for reg
                if matched_reg is not None:
                    # calculate the bias of the ground truth object boxes
                    # with respect to the predictions of the still object detector
                    matched_reg_targets = self.encode_reg_preds(refined_reg_targets,
                                                                original_pred_result[refined_obj_masks])
                    if matched_reg.dim() == 3:
                        loss_matched_iou = 0
                        for i in range(matched_reg.size(0)):
                            loss_matched_iou += (
                                                    self.smooth_l1_loss(matched_reg[i].view(-1, 4)[refined_obj_masks],
                                                                        matched_reg_targets)
                                                ).sum() / num_fg_local
                        loss_matched_iou /= matched_reg.size(0)
                    else:
                        loss_matched_iou = (
                                               self.smooth_l1_loss(matched_reg.view(-1, 4)[refined_obj_masks],
                                                                   matched_reg_targets)
                                           ).sum() / num_fg_local
                else:
                    loss_matched_iou = 0
            ...
            # bce loss for cls
            if self.kwargs.get('cls_ota', True):
                if refined_cls.dim() == 3:
                    loss_refined_cls = 0
                    for i in range(refined_cls.size(0)):
                        loss_refined_cls += (
                                                self.bcewithlog_loss(
                                                    refined_cls[i].view(-1, self.num_classes)[refined_obj_masks],
                                                    refined_cls_targets
                                                )
                                            ).sum() / num_fg_local
                    loss_refined_cls /= refined_cls.size(0)
                else:
                    loss_refined_cls = (
                                           self.bcewithlog_loss(
        ...
        if loss_matched_obj > 15:
            loss_matched_obj = loss_matched_obj / float(loss_matched_obj) * 15

        loss = (reg_weight * loss_iou + loss_obj + loss_l1 + loss_cls
                + loss_refined_cls + loss_matched_obj + iou_match_weight * loss_matched_iou)
        ...
        return (
            ...
            # loss_refined_obj,
            loss_matched_obj,
            iou_match_weight * loss_matched_iou
        )
    ...
    def get_losses(
        ...
        """
            add bce loss for video classification, bce loss for video localization based on yolox loss
        """
        bbox_preds = outputs[:, :, :4]  # [batch, n_anchors_all, 4]
        obj_preds = outputs[:, :, 4].unsqueeze(-1)  # [batch, n_anchors_all, 1]
        cls_preds = outputs[:, :, 5:]  # [batch, n_anchors_all, n_cls]
    ...
    def postprocess_widx(self, prediction, num_classes, nms_thre=0.5, ota_idxs=None, conf_thresh=0.001):
        ...
        Returns:
            [batch,topK,5+clsnum]
        '''
        # (x, y, w, h) center size -> (x1, y1, x2, y2)
