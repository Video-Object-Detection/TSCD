...
class Predictor(object):
    ...
    def visual(self, output, img_info, cls_conf=0.35):
        ...
        vis_res = vis(img, bboxes, scores, cls, cls_conf, self.cls_names,t_size=2)
        return vis_res
    ...
    while True:
        ...
        if ret_val:
            ...
            if args.save_result:
                ...
                cv2.imwrite(os.path.join(save_folder, str(img_idx) + '.jpg'), result_frame)
            ch = cv2.waitKey(1)
            if ch == 27 or ch == ord("q") or ch == ord("Q"):
                break
        else:
...
def main(exp, args):
    ...
    if args.dataset == 'vid':
        predictor = Predictor(
            model, exp, VID_classes, trt_file, decoder,
            args.device, args.fp16, args.legacy,
        )
    elif args.dataset == 'ovis':
        predictor = Predictor(
            model, exp, OVIS_classes, trt_file, decoder,
            args.device, args.fp16, args.legacy,
        )
