...
def make_parser():
    ...
    parser.add_argument(
        "-f",
        "--exp_file",
        default='exps/yolov++/v++_base_decoupleReg.py',
        type=str,
        help="pls input your expriment description file",
    )
    parser.add_argument("-c", "--ckpt", default='weights/v++_s.pth', type=str, help="ckpt for eval")
    parser.add_argument(
        "--device",
        default="gpu",
        type=str,
    ...
    parser.add_argument('--output_dir', default='./V++_outputs/v++_s.pkl',
...
class Predictor(object):
    def __init__(
        ...
        self.post = post
    def inference(self, img, time_embedding=None, img_path=None, lframe=0, gframe=32, resume=False):
        ...
        if self.device == "gpu":
            ...
            if time_embedding is not None:
                time_embedding = time_embedding.to(img)

        with torch.no_grad():
            t0 = time.time()
            if not self.traj_linking:
                outputs, outputs_ori = self.model(img, time_embedding=time_embedding, lframe=lframe, gframe=gframe, resume=resume)
                if lframe == 0 or gframe == 0:
...
def imageflow_demo(predictor, val_path, args,exp):
    ...
    for cur_iter, (imgs, _, info_imgs, label, path, time_embedding) in enumerate(
        ...
        #     pred_results, adj_lists, fc_outputs,names = [], [], [],[]
        if exp.lmode and exp.gmode:
            if args.lframe == 0 or int(path[0].split('/')[-1].split('.')[0]) == 0:
                resume = False
            else:
                resume = True
            pred_res = predictor.inference(imgs, time_embedding=time_embedding, lframe=args.lframe, gframe=args.gframe, resume=resume)
        elif exp.lmode:
            if traj_linking:
