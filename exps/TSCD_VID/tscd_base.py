...
class Exp(BaseExp):
    def __init__(self):
        ...
        # path to vid image list
        self.vid_train_path = './yolox/data/datasets/train_seq.npy'
        self.vid_val_path = './yolox/data/datasets/val_seq.npy'
        # key-value pairs of vid image path and vid annotation information
        self.vid_train_anno_cache_path = './yolox/data/datasets/cache/VID_train_anno.pkl'
        self.vid_val_anno_cache_path = './yolox/data/datasets/cache/VID_val_anno.pkl'
    ...
    def get_data_loader(
        ...
        dataset = vid.VIDDataset(file_path=self.vid_train_path,
                                 ...
                                 training=True,
                                 cache_file=self.vid_train_anno_cache_path
                                 )
        if self.use_aug:
    ...
    #         size_factor = self.input_size[1] * 1.0 / self.input_size[0]
    #         if not hasattr(self, 'random_size'):
    #             min_size = int(self.input_size[0] / 128) - self.multiscale_range
    #             max_size = int(self.input_size[0] / 128) + self.multiscale_range
    #             self.random_size = (min_size, max_size)
    #         size = random.randint(*self.random_size)
    #         size = (int(128 * size), 128 * int(size * size_factor))
    #         tensor[0] = size[0]
    #         tensor[1] = size[1]
    #
    #     if is_distributed:
    ...
    def get_eval_loader(self, batch_size, tnum=None, data_num_workers=8, formal=False):
        ...
        dataset_val = vid.VIDDataset(file_path=self.vid_val_path,
                                     ...
                                     gframe=self.gframe_val, val=True, dataset_pth=self.data_dir, tnum=tnum,
                                     formal=formal,
                                     traj_linking=self.traj_linking, local_stride=self.local_stride, training=False,
                                     cache_file=self.vid_val_anno_cache_path)
