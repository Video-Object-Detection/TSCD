...
sys.path.append("..")
from exps.TSCD_OVIS.tscd_base import Exp as MyExp
...
class Exp(MyExp):
    def __init__(self):
        ...
        self.lframe_val = 8
        self.gframe = 12
        self.gframe_val = 24  # config your gframe_val and gframe here
        self.use_loc_emd = True
        self.iou_base = False
        self.reconf = True
        self.loc_fuse_type = 'identity'
        self.output_dir = "./OVIS_TSCD_outputs/TSCD_Baseline"
    ...
    def get_model(self):
        ...
        from yolox.models.tscd_head import TSCDHead
        from yolox.models.tscd import TSCD
        ...
        head = TSCDHead(self.num_classes, self.width, in_channels=in_channels, heads=self.head,
                        drop=self.drop_rate,
                        use_score=self.use_score, defualt_p=self.defualt_p,
                        sim_thresh=self.sim_thresh,
                        pre_nms=self.pre_nms, ave=self.ave, defulat_pre=self.defualt_pre,
                        test_conf=self.test_conf,
                        use_mask=self.use_mask, gmode=self.gmode, lmode=self.lmode,
